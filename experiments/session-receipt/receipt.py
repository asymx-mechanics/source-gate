#!/usr/bin/env python3
"""REJECTED after adversarial testing (see NOTES.md). Kept only as the specimen attacks.py runs against.

Original intent: record what a working session changed in a git repository, in a form someone
else can re-check. It cannot deliver that; its "unchanged" and "OK" are not evidence.
"""
...
def file_hashes(repo):
    # ls-files only reads; `git status` would be simpler but may rewrite the index while observing it.
    listed = git(repo, "ls-files", "--cached", "--others", "--exclude-standard", "-z")
    hashes = {}
    for rel in filter(None, listed.split("\0")):
        path = Path(repo, rel)
        # --cached still lists tracked files that were deleted from disk
        if path.is_file():
