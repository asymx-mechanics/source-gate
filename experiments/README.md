...
| `reproducible-tools/` | Can a later session re-run this evidence with the same tools, and does the evidence depend on them? | **Held so far.** Every case gives the same outcome under the container's tools and under a toolchain pinned with Nix (`env.nix`). A later session, on a machine without Nix, got the same store paths (generation 1). |
