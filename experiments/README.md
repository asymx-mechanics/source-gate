...
| `checkable-memory/` | Can a later session check what it is told to remember? | **Partly answered.** Of ten remembered facts, seven can be re-checked by a later session and three rest on this session's record; none said which. A check now asks every fact for a pointer or a "seen only". Shortening `CLAUDE.md` silently strengthened four notes; comparing with the unchanged ring (`../memory/`) caught it. Whether a later session reads the ring is untested. |
