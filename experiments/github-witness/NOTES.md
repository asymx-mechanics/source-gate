...
**Afterwards: the pull request for this branch** (asymx-mechanics/source-gate#1, 11:30:28 UTC). [V]

- The PR is recorded as created by `asymx-mechanics` (`author_association: OWNER`), **and**
  `performed_via_github_app: {slug: claude, id: 1236702, client_id: Iv23liqTIFEtdIu6Vn1r}`.
  So GitHub *does* name the app on issue and PR objects, while the push activity
  record has no such field.
- The client id matches the token the proxy injects. The GitHub tools and the
  direct HTTP/git path act through the same app.
- The PR description on GitHub ends with a footer I did not write ("Generated by
